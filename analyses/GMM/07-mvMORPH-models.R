...
### Brownian motion
fitBMgmm <- mvBM(trees.SYM, pcdata, model = "BM1", method = "sparse")

### Single optimum OU
fitOUgmm <- mvOU(trees.SYM, pcdata, model = "OU1", method = "sparse")

### Multi-rate Brownian motion models (BMS)
# Uses the non-censored model of O'Meara et al. 2006 by default
fitBMSgmm <- mvBM(trees.SYM, pcdata, model = "BMM", method = "sparse")

### Multi-optima OU models (OUM) with constant alpha and sigma
fitOUMgmm <- mvOU(trees.SYM, pcdata, model = "OUM", method = "sparse", 
               param= list(alpha = "constraint", sigma = "constraint"))

### Multi-rate multi-optima OU motion models (OUMV)
fitOUMVgmm <- mvOU(trees.SYM, pcdata, model = "OUM", method = "sparse", 
...
resultsgmm <- c(AICc(fitBMgmm), AICc(fitOUgmm), AICc(fitBMSgmm), AICc(fitOUMgmm), AICc(fitOUMVgmm))

# Get aic weights
aicwgmm <- aicw(resultsgmm)
